...
from humarconoid.envs import ARCManagerBasedRLEnv


class ArcRlVecEnvWrapper(VecEnv):
    ...
    def __init__(self, env: ARCManagerBasedRLEnv | ManagerBasedRLEnv | DirectRLEnv, clip_actions: float | None = None):
        ...
        Raises:
            ValueError: When the environment is not an instance of :class:`ARCManagerBasedRLEnv` or :class:`ManagerBasedRLEnv` or :class:`DirectRLEnv`.
        """
        # check that input is valid
        if not isinstance(env.unwrapped, ARCManagerBasedRLEnv) and not isinstance(env.unwrapped, ManagerBasedRLEnv) and not isinstance(env.unwrapped, DirectRLEnv):
            raise ValueError(
                "The environment must be inherited from ARCManagerBasedRLEnv or ManagerBasedRLEnv or DirectRLEnv. Environment type:"
                f" {type(env)}"
            )
    ...
    @property
    def unwrapped(self) -> ARCManagerBasedRLEnv | ManagerBasedRLEnv | DirectRLEnv:
